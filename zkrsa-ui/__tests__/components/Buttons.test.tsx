import { ButtonGenerateProof, ButtonExportProof, ButtonInitializeVerifier } from "../../src/components/Buttons";
...
describe("Testing buttons", () => {
    describe("ButtonGenerateProof", () => {
        it("disables button when vkeyProof && hash && signature && publicKey is falsy", () => {
            ...
            const { container } = render(<ButtonGenerateProof
                setpublicSignals={setpublicSignals} setproof={setproof}
                setcompiledCircuit={setcompiledCircuit} hash={''}
                signature={null} publicKey={""}
                vkeyProof={""} vkeyVerifier={""}
            />);
        ...
        it("enables button when vkeyProof && hash && signature && publicKey is truthy", () => {
            const { container } = render(<ButtonGenerateProof
                setpublicSignals={setpublicSignals} setproof={setproof}
                setcompiledCircuit={setcompiledCircuit} hash={"someHash"}
                signature={"someSignature"} publicKey={"somePublicKey"}
                vkeyProof={"someProof"} vkeyVerifier={""}
            />);
            const button = container.querySelector("button");
    ...
    describe("Testing ButtonExportProof", () => {
        it("displays a 'Download' text when there are proof and public signals", () => {
            const { container } = render(<ButtonExportProof proof={"proof"} publicSignals={"publicSignal"} />);
        ...
        it("does not display anything when there are no proof and public signals", () => {
            const { container } = render(<ButtonExportProof proof={null} publicSignals={null} />);
            expect(container.children).toHaveLength(0);
        });
    });
    describe("Testing ButtonInitializeVerifier", () => {
        it("displays a 'Initialize' text button when not loading", () => {
            const { container } = render(<ButtonInitializeVerifier
                setvkeyState={undefined} setvkeyProof={undefined}
